    ...
    <div className="min-h-screen w-full bg-[var(--bg)] text-[var(--text)]">
      ...
      <main className="ml-72 p-6">
        <header className="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--bg-elev)] p-5">
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">Your complete focus room history from Firestore.</p>
        </header>
          ...
          {records.map((session) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h2 className="font-medium text-[var(--accent)]">{session.roomTitle}</h2>
                <p className="text-sm text-[var(--muted)]">{formatDate(session.startTime.toMillis())}</p>
              </div>
              <p className="mt-2 text-sm text-[var(--muted)]">Duration: {session.duration} min</p>
            </article>
          ))}
        </section>
      </main>
